        ...
        while True:
            ...
            if gotpkt:
                ...
                if(arp):
                    ...
                    log_info("Arp Table {}".format(arpTable))
                    ...
                    for intf in my_interfaces:
                        if arp.targetprotoaddr == intf.ipaddr:
                            ...
                            if arp.operation == ArpOperation.Request:
                                ...
                                # Get the packet from WaitQueue and if we have reply, forward the IP Packet
                ...
                # Send the ARP Requests again for packets that are in WaitQueue.
                QDestIP = {}
                for pktsInQ in WaitQueue:
                    QDestIP[pktsInQ.pkt[1].dst] = False
                # The above for loop is just to avoid re-sending of ARP Req for same packets.
                ...
                if (ipv4_header):
                    log_debug("Got a IPv4 packet with header as: {}".format(str(ipv4_header)))
                    ...
                    if GotEntry == True:
                        ...
                        log_info("ArpTable is {}".format(arpTable))
            ...
            # Else If no packets recv, still send ARP Req for these packets
            else:
                # Send the ARP Requests again for packets that are in WaitQueue.
                QDestIP = {}
                for pktsInQ in WaitQueue:
                ...
                for pktsInQ in WaitQueue:
                    ...
                    # Send the ARP Request Again in timmeout
                    if QDestIP[pktsInQ.pkt[1].dst] == False:
                        log_info("Send an ARP Req from srchw {} srcIP {} destIP {}".format(pktsInQ.srcHw, pktsInQ.srcIp, pktsInQ.pkt[1].dst))
                        ArpReqPkt = self.CreateArpPacket(srcHw, srcIp, pktsInQ.pkt[1].dst)
                        net.send_packet(pktsInQ.Intf, ArpReqPkt)
